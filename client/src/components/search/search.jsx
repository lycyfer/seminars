...
import apiRequest from "../../lib/apiRequest"; // Импортируем ваш axios-клиент
...
const Search = () => {
  ...
  const fetchSeminars = async (query) => {
    setLoading(true);
    setError(null);

    try {
      const url = query.trim()
        ? `/seminars?title_like=${encodeURIComponent(query)}`
        : "/seminars";
  ...
  const handleInputChange = (e) => {
    ...
    // Выполняем поиск при изменении текста
    if (value.trim()) {
      fetchSeminars(value);
    } else {
      fetchSeminars("");
  ...
  return (
    <div className="search">
      ...
      {loading && <div className="response-block visible">Загрузка...</div>}

      {isVisible && !loading && seminars.length > 0 && (
        <div className="response-block visible">
          {seminars.map((seminar) => (
            <Link
              to={`/list`}
              className="response-block-item"
              key={seminar.id}
            >
              <img
                src={seminar.photo}
              ...
              <div className="response-block-item-des">
                <div className="response-block-item-title">{seminar.title}</div>
                <div className="response-block-bottom">
                  <div className="response-block-item-capacity">
                    ...
                    <div className="response-capacity">{seminar.time}</div>
                  </div>
                  <div className="response-block-item-large">
                    Date {seminar.date}
                  </div>
                </div>
              </div>
